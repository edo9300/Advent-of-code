...
parse_command() {
	if [ $1 = "$" ]; then
		...
		echo "$cur_path${2}/ _ 0" >> /tmp/aoc_2022_day_7_part_1_files
	else
...
parse_folder() {
	...
	size=$2
	get_base_path $1
	printf "%s %012d\n" $base_path $size >> /tmp/aoc_2022_day_7_part_1_files
...
longest_path=$(sed 's/[^/]*//g' /tmp/aoc_2022_day_7_part_1_files | awk '{print length}'|sort -nr|head -1)
...
$(sed -e 's/ 0*/ /' -e '/^$/d' /tmp/cur_input | awk -FN -vcount=$longest_path 'BEGIN { FS = "/" } ;NF==count+1')
EOF
	if [ -n "$cur_path" ]; then
		printf "%s %012d\n" $cur_path $total >> /tmp/aoc_2022_day_7_part_1_files
	fi
	awk -FN -vcount=$longest_path 'BEGIN { FS = "/" } ;NF<=count' /tmp/cur_input >> /tmp/aoc_2022_day_7_part_1_files
	sort -r -o /tmp/aoc_2022_day_7_part_1_files /tmp/aoc_2022_day_7_part_1_files
	cat /tmp/aoc_2022_day_7_part_1_files >> /tmp/aoc_2022_day_7_part_1_files_totals
...
awk '!_[$1]++' /tmp/aoc_2022_day_7_part_1_files_totals | sed '/^$/d' > /tmp/aoc_2022_day_7_part_1_files_totals_uniq
...
used_space=$(awk '{ print $2 }' /tmp/aoc_2022_day_7_part_1_files_totals_uniq | sort | tail -n1 | sed 's/^0*//')
...
printf "Part 2: %d\n" "$(awk -v space_to_free="$space_to_free" '$2 >= space_to_free { print $2 }' /tmp/aoc_2022_day_7_part_1_files_totals_uniq  | sort | head -n1 | sed 's/^0*//')"
